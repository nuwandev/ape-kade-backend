    ...
    public List<Category> search(String query) {
        ...
        String sql = """
                    ...
                    WHERE LOWER(c.display_name) LIKE ? 
                       OR LOWER(c.tagline) LIKE ? 
                       OR LOWER(c.slug) LIKE ? 
                       OR LOWER(c.seo_description) LIKE ?
                    GROUP BY c.id, c.display_name, c.tagline, c.slug, c.visibility, c.icon, c.seo_description, c.created_at
